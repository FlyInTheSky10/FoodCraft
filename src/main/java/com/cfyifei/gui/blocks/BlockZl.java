...
public class BlockZl extends BlockContainer {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!EE) {
            ...
            if (tileentity instanceof TileEntityZl) {
                InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityZl)tileentity);
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos) {
        ...
        if (active) {
            worldIn.setBlockState(pos, ModGui.lit_Zl.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, ModGui.lit_Zl.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        }
        else {
            worldIn.setBlockState(pos, ModGui.Zl.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, ModGui.Zl.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        }
        EE = false;
        if (tileentity != null) {
